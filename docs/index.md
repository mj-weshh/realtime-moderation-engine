...
!!! info "Project status"
    **Week 1 complete** — core infrastructure (Kafka in KRaft mode, Kafka UI, Neo4j) and the containerized data producer are live. The ML inference consumer, WebSocket API, and Next.js frontend are under active development.
...
flowchart LR
    ...
    kafka -. "Week 2" .-> consumer[ml_consumer<br/>toxicity inference]
...
2. The **ML consumer** *(Week 2)* runs batched transformer inference, writes conversation graphs to Neo4j, and re-publishes scored payloads.
...
**Python 3.13** · **Apache Kafka (KRaft)** · **Neo4j 5** · **Docker Compose** · *upcoming:* PyTorch + Hugging Face Transformers, Node.js WebSockets, Next.js + Tailwind
