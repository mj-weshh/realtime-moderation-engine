...
|---|---|---|---|
| `raw_comments` | `producer_service` | `ml_consumer` | **Live** |
| `scored_comments` | `ml_consumer` | `websocket_api` (source: `backend_api/`) | **Live** |
...
The ML consumer pulls batches of up to 16 messages and commits offsets only after Neo4j and `scored_comments` writes succeed. See [ML Inference](ml_inference.md) for the full loop.

The `websocket_api` service consumes `scored_comments` in consumer group `websocket-bridge` with `fromBeginning: false` — only new messages after connect. Benign payloads are filtered at the bridge before WebSocket broadcast. See [WebSocket API](websocket_api.md).
...
The ML consumer republishes each message with inference results appended:
...
| Field | Type | Description |
|---|---|---|
| `scores` | `object` | Six toxicity dimensions from `unitary/toxic-bert` (sigmoid probabilities, rounded to 4 decimals) |
| `is_flagged` | `boolean` | `true` when `scores.toxicity >= FLAG_THRESHOLD` (default `0.5`) |
