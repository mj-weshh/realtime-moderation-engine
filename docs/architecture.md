...
The system is built as **decoupled, containerized microservices** connected by an event streaming backbone. No service calls another directly; everything flows through Kafka topics. This page explains the design decisions behind the infrastructure and the live ML consumer.
...
| `PLAINTEXT` | `kafka:29092` | Containers on `moderation_network` (producer, ml_consumer, Kafka UI) |
